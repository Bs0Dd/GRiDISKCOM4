...
int checkFreeSp(uint8_t* data, size_t data_size, vector<ccos_inode_t*> inodeList,
    ...
    size_t fre = ccos_calc_free_space(data, data_size);
    *needs = 0;
    for (int i = 0; i < calledElems.size(); i+=6){
        ...
        if (file != 0){
            if (ccos_is_dir(file)){
                ...
                ccos_get_dir_contents(file, data, &fils, &dirdata);
...
int dump_file_qt(ccos_inode_t* file, uint8_t* data, QString path, QWidget* parent){
...
int dump_dir_qt(ccos_inode_t* dir, uint8_t* data, QString path, QWidget* parent){
    ...
    uint16_t fils = 0;
    ccos_inode_t** dirdata = NULL;
    ccos_get_dir_contents(dir, data, &fils, &dirdata);

    for(int i = 0; i < fils; i++){
        if (ccos_is_dir(dirdata[i]))
            dump_dir_qt(dirdata[i], data, dpath, parent);
        else
            dump_file_qt(dirdata[i], data, dpath, parent);
    }
...
int dump_img_qt(uint8_t* data, size_t data_size, QString path, QString altname, QWidget* parent){
    ...
    return dump_dir_qt(root_dir, data, dpath, parent);
}
...
void MainWindow::AddDirs(QStringList dirs){
    ...
    for (int i = 0; i < dirs.size(); i++){
        ...
        ccos_inode_t* newdir = ccos_create_dir(root, QFileInfo(dirs[i]).fileName().toStdString().c_str(),
...
int MainWindow::AddFiles(QStringList files, ccos_inode_t* copyTo){
    ...
    for (int i = 0; i < files.size(); i++){
        ...
        if (read_file_qt(files[i], &fdat, &fsiz, this) == 0){
            if (ccos_add_file(copyTo, fdat, fsiz, fname.c_str(), dat[acdisk], siz[acdisk]) == NULL){
...
void MainWindow::AnotherPart(bool fromMenu){
    ...
    if (chsd->exec() == 1){
        ...
        ccos_inode_t* root = ccos_get_root_dir(dat[topan], siz[topan]);
        if (root == NULL){
        ...
        fillTable(root, 0, dat[topan], siz[topan], topan, ui);
    }
}
...
void MainWindow::CopyLoc(){
    if (isop[acdisk]){
        ...
        if (nrot[acdisk]){
            ...
            ccos_inode_t* firfil = inodeon[acdisk][called[0]->row()] == 0 ?
                        inodeon[acdisk][called[6]->row()] : inodeon[acdisk][called[0]->row()];

            if (dirdata[chsd->getIndex()]->header.file_id == firfil->dir_file_id){
            ...
            for (int t = 0; t < called.size(); t+=7){
                if (inodeon[acdisk][called[t]->row()]==0)
                    continue;
                ccos_copy_file(dat[acdisk], siz[acdisk], dirdata[chsd->getIndex()],
            ...
            for (int t = 0; t < called.size(); t+=7){
                ...
                while (true){
                    ...
                    if (name == "")
                        ...
                        ccos_inode_t* newdir = ccos_create_dir(root, name.toStdString().c_str(), dat[acdisk], siz[acdisk]);
                        if (newdir == NULL){
                            QMessageBox::critical(this, "Failed to create folder",
                                            "Program can't create a folder in the image!");
                            break;
                        ...
                        for(int i = 0; i < fils; i++){
                            ccos_copy_file(dat[acdisk], siz[acdisk], newdir,
    ...
    if (isop[acdisk]){
        ...
        for (int t = 0; t < called.size(); t+=7){
            if (inodeon[acdisk][called[t]->row()]==0)
                continue;
            if (ccos_is_dir(inodeon[acdisk][called[t]->row()]))
                dump_dir_qt(inodeon[acdisk][called[t]->row()], dat[acdisk], todir, this);
            else
                dump_file_qt(inodeon[acdisk][called[t]->row()], dat[acdisk], todir, this);
        }
    }
}
...
void MainWindow::ExtractAll(){
    if (isop[acdisk]){
        ...
        if (todir == "")
            return;
        int res = dump_img_qt(dat[acdisk], siz[acdisk], todir, QFileInfo(name[acdisk]).baseName(), this);
        if (res == -1){
    ...
    if (root == NULL){
        if (siz[acdisk] > 0x200 && dat[acdisk][0x1FE] == 0x55 && dat[acdisk][0x1FF] == 0xAA){ //MBR detected
            ...
            if (chsd->exec() == 1){
                ...
                root = ccos_get_root_dir(dat[acdisk], siz[acdisk]);
                if (root == NULL){
                    QMessageBox::critical(this, "Incorrect Image File",
                                          "Image broken or have non-GRiD format!");
                    free(hdddat[acdisk]);
            ...
            while (true){
                if (rnam->exec() == 1){
                    ...
                    if (newtype.contains("subject", Qt::CaseInsensitive) && nrot[acdisk]){
                        ...
                        ccos_rename_file(dat[acdisk], siz[acdisk], reninode, newname.toStdString().c_str(),
